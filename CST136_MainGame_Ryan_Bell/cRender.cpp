...
bool cRender::LoadMedia()
{
	...
	char* const texturePath[] = { "BackSplashYellow.bmp",
		"Bubbles.bmp",
		"DirectionArrow.bmp" }; //BMP image paths
	...
	for (int i = 0; i < totalTextures && mediaSuccess == true; i++)
		//For each image that needs to be loaded
	{
		tempSurface = SDL_LoadBMP(texturePath[i]);
